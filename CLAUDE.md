...
Authenticated API documentation site built with Zudoku. Features Clerk authentication, dynamic credential management (Keycloak or environment variables), and direct API testing with automatic Bearer token injection.
...
**Multi-workspace:** Prefix credentials with lowercase workspace name (e.g., `acme_AUTH_CLIENT_ID`)
...
Add to `src/lib/utils.ts` for shared logic. Keep functions pure and well-typed.

### Adding Components

React components go in `src/components/`. Use TypeScript and proper prop typing.

### Adding Pages

```tsx
// 1. Create component in src/MyPage.tsx
export const MyPage: React.FC = () => <div>Content</div>;

// 2. Add to zudoku.config.tsx navigation
{
  type: "custom-page",
  path: "/my-page",
  label: "My Page",
...
Add routes in `server.ts` after line 174. Follow existing error handling patterns.
...
| Circular dependency errors | Wrong import path | Ensure `token.ts` imports only from `utils`, never from `http` |

### Quick Debug Commands

```bash
# Check CORS headers present
curl -I -X OPTIONS -H "Origin: http://localhost:3000" https://api.finarkein.in/factory/v1/endpoint

# Decode JWT token (paste actual token)
echo "eyJhbGc..." | cut -d'.' -f2 | base64 -d | jq .
```

### Common Mistakes

- Not handling OPTIONS preflight requests (always return 204)
- Forgetting to add both `localhost:3000` and `127.0.0.1:3000`
- Using `Access-Control-Allow-Origin: *` with credentials (use specific origins)

## Important Constraints

- **No localStorage:** Credentials/tokens stay in memory only for security
- **Workspace isolation:** Each workspace has separate credentials and token cache
- **Module type:** Project uses ES modules (`"type": "module"` in package.json)
- **Import extensions:** Always use `.js` extension in imports, even for `.ts` files (e.g., `from './lib/http.js'`)
